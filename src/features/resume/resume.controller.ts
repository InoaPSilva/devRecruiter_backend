...
export class ResumeController {
  ...
  async update(
    ...
    try {
      const updateResume = await this.resumeService.update(+id, updateResumeDto);
      return <Response>(
        res.status(HttpStatus.OK).json({ message: updateResume })
      );
    } catch (err) {
      console.log(err);
      return <Response>res.status(HttpStatus.BAD_REQUEST).json({ error: err });
  ...
  async remove(
    ...
    try {
      ...
      return <Response>(
        res.status(HttpStatus.OK).json({ message: "Resume delete" })
      );
    } catch (err) {
      console.log(err);
      return <Response>res.status(HttpStatus.BAD_REQUEST).json({ error: err });
